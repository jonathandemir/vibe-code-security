...
async function scanCurrentFile() {
  ...
  vscode.window.withProgress({
    ...
    try {
      const response = await axios_default.post(`${apiUrl}/scan`, {
        code,
        language
      }, {
        headers: {
...
function updateDiagnostics(document2, issues) {
  const diagnostics = [];
  issues.forEach((issue) => {
    let lineIdx = 0;
    if (issue.vulnerable_code_snippet) {
      const lines = document2.getText().split("\n");
      const targetLine = issue.vulnerable_code_snippet.split("\n")[0].trim();
      lineIdx = lines.findIndex((l) => l.includes(targetLine));
      if (lineIdx === -1)
        lineIdx = 0;
    }
    const range = new vscode.Range(lineIdx, 0, lineIdx, document2.lineAt(lineIdx).text.length);
...
var VibeGuardFixProvider = class {
  ...
  provideCodeActions(document2, range, context) {
    ...
    context.diagnostics.filter((diagnostic) => diagnostic.source === "VibeGuard").forEach((diagnostic) => {
      ...
      if (fixedCode) {
        const action = new vscode.CodeAction("\u{1F6E1}\uFE0F VibeGuard: Apply Recommended Fix", vscode.CodeActionKind.QuickFix);
        action.edit = new vscode.WorkspaceEdit();
        action.edit.replace(document2.uri, diagnostic.range, fixedCode);
